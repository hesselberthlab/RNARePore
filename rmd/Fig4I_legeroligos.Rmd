...
library(ggsignif)
library(here)
here::i_am("rmd/Fig4I_legeroligos.Rmd")
```
## Background
...
```{r}
read_delim(here("data/download_zenodo/Leger_eventalign/Oligo_control_623402.eventalign_output.txt"), delim = "\t", col_names = T) %>%
...
read_delim(here("data/download_zenodo/Leger_eventalign/Oligo_1_623403.eventalign_output.txt"), delim = "\t", col_names = T) %>%
...
read_delim(here("data/download_zenodo/Leger_eventalign/Oligo_2_623404.eventalign_output.txt"), delim = "\t", col_names = T) %>%
...
read_delim(here("data/download_zenodo/Leger_eventalign/Oligo_3_623405.eventalign_output.txt"), delim = "\t", col_names = T) %>%
...
Calculate and plot mean differences in dwell time. 
Note: the code below takes several objects generated by `Fig4I_Stephenson_reanalysis.Rmd`. Run that first if you want to be able to patch the full plots together, including the modifications from the ribosomal RNA. 
```{r}
...
hac %>% ungroup() %>% mutate(modification = "2′-P") -> hac
