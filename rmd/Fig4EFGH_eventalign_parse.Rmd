...
```{r}
read_delim(here("data/download_zenodo/oligos_eventalign/ctrl.eventalign_output.txt"), delim = "\t", col_names = T) %>%
  mutate(id = "unmodified") %>%
  select(-strand, -contig, -model_kmer, -model_mean, -model_stdv, -standardized_level) -> ctrlsplint

read_delim(here("data/download_zenodo/oligos_eventalign/2P.eventalign_output.txt"), delim = "\t", col_names = T) %>%
  mutate(id = "modified")  %>%
