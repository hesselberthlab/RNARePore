...
library(inborutils)
library(here)
here::i_am("rmd/SuppFigs_syntheticoligos.Rmd")
```
# these called for *.bwa.eventalign* (except 5050), are thse correct?
# no we need the bwa alignments
# reversion Zenodo, siiiiigh
```{r}
read_delim(here("data/download_zenodo/oligos_eventalign/ctrl.bwa.eventalign_output.txt"), delim = "\t") %>%
  mutate(id = "unmodified") %>%
  select(-strand, -contig, -model_kmer, -model_mean, -model_stdv, -standardized_level) -> ctrlsplint

read_delim(here("data/download_zenodo/oligos_eventalign/2P.bwa.eventalign_output.txt"), delim = "\t") %>%
  mutate(id = "modified")  %>%
  select(-strand, -contig, -model_kmer, -model_mean, -model_stdv, -standardized_level) -> modsplint

read_delim(here("data/download_zenodo/oligos_eventalign/5050.eventalign_output.txt"), delim = "\t") %>%
  mutate(id = "mixed")  %>%
...
test %>%
  ...
  geom_signif(comparisons = list(c("modified", "mixed"), c("modified", "unmodified"), c("unmodified", "mixed")),
              ...
              step_increase = .2)
```
Now, the end reason analysis. To run this, you should have first run the FileDownloader.Rmd script.
