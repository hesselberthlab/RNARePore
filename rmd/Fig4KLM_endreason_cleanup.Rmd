...
here::i_am("rmd/Fig4KLM_endreason_cleanup.Rmd")
```
## Background
...
file_loc = dir_ls(path = here('data/download_zenodo/mRNA_seq_summaries/'))
...
sum_table <- purrr::map_df(file_loc, read_file)

sum_table %>%
  select(-pore_type, -strand_score_template, -experiment_id) %>%
  select(-run_id, -channel, -filename_fastq, -filename_fast5, -sample_id) %>%
  separate(name, into = c("genotype", "treatment", "enzyme"), extra="drop") %>%
  mutate(genotype = replace(genotype, genotype != "CRY1", "mutant")) %>%
  mutate(genotype = replace(genotype, genotype != "mutant", "wild-type")) -> sum_table
```
...
```{r}
file_loc = dir_ls(path = here('data/read_lookups/'))

read_lookup <- function(x) {
  df <- readr::read_delim(x, delim = "\t", col_names = c("read_id", "ref"))
  df$name <- basename(x)
  
  df
  }
  
purrr::map_df(file_loc, read_lookup) %>%
  separate(name, into = c("genotype", "treatment", "enzyme"), extra="drop", sep = "_") %>%
  separate(enzyme, into = c("enzyme"), extra = "drop") %>%
  mutate(genotype = replace(genotype, genotype != "CRY1", "mutant")) %>%
  mutate(genotype = replace(genotype, genotype != "mutant", "wild-type"))-> allreads

####### WHY IS ALLREADS SMALLER THAN SUMTABLE OMG KILL ME
# One issue: don't seem to have mutant correctly
left_join(allreads, sum_table, by = c("read_id", "genotype", "treatment", "enzyme")) %>%
