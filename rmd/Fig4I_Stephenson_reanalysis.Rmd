...
read_delim(here("data/download_zenodo/Stephenson_eventalign/25S.2220.native.eventalign.subset.txt"), delim = "\t",
           col_names = c("contig", "position",	"reference_kmer",	"read_id",	"event_index",	"event_level_mean",	"event_length"))  %>%
  mutate(position = position +3) %>%
  group_by(contig, position, read_id) %>%
  summarise(dwell = sum(event_length, na.rm = TRUE)*1000) %>%
...
read_delim(here("data/download_zenodo/Stephenson_eventalign/18S.1428.ivt.eventalign.subset.txt"), delim = "\t",
           col_names = c("contig", "position",	"reference_kmer",	"read_id",	"event_index",	"event_level_mean",	"event_length"))  %>%
  mutate(position = position +3) %>%
  group_by(contig, position, read_id) %>%
  summarise(dwell = sum(event_length, na.rm = TRUE)*1000) %>%
...
read_delim(here("data/download_zenodo/Stephenson_eventalign/18S.1428.native.eventalign.subset.txt"), delim = "\t",
           col_names = c("contig", "position",	"reference_kmer",	"read_id",	"event_index",	"event_level_mean",	"event_length"))  %>%
  mutate(position = position +3) %>%
  group_by(contig, position, read_id) %>%
  summarise(dwell = sum(event_length, na.rm = TRUE)*1000) %>%
